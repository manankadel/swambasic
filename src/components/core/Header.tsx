...
export const Header = () => {
    ...
    return (
        <>
            <header className="fixed top-0 left-0 w-full z-50 p-4 md:p-6">
                <div className="mx-auto max-w-7xl px-4 md:px-6 py-4 rounded-3xl border border-white/10 bg-black/20 backdrop-blur-sm">
                    <nav className="relative flex items-center justify-between w-full">
                        ...
                        {/* --- THIS IS THE FIX --- */}
                        {/* I have added the `-ml-4` class to shift the logo slightly to the left. */}
                        {/* You can change this value (e.g., -ml-2, -ml-6) to get the exact position you want. */}
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 -ml-4">
                            <Link href="/home">
                                <span className="font-display font-bold text-white text-xl md:text-2xl" style={{ textShadow: '0 0 15px rgba(255, 255, 255, 1)' }}>
                                    SWAMBASIC
                                </span>
                            </Link>
                        </div>
            ...
            <AnimatePresence>
                {isMenuOpen && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-lg z-[100] p-6">
                        <div className="flex justify-end"> <button onClick={() => setIsMenuOpen(false)} className="text-white"><CloseIcon /></button> </div>
                        <nav className="flex flex-col items-center justify-center h-full gap-8 text-white font-display text-3xl uppercase">
                            ...
                            <Link href="/reach-out" onClick={() => setIsMenuOpen(false)}>Reach Out</Link>
                        </nav>
                    </motion.div>
                )}
            </AnimatePresence>
